...
module.exports = {
    ...
    run: async(client, message, args, container) => {
        ...
        .setDescription(`
        Please enter the id of the user you would like to resign.

        **E.G | 234273606043959296**
        ...
        *Note: Type cancel to quit.*`)
        ...
        message.channel.awaitMessages({filter, max: 1, time: 10000
            ...
            if (collected.first().content.toLowerCase() == 'cancel') {
                ...
                .setColor(config.serverColor)
                .setAuthor({name: `${config.serverName} Resign`, iconURL: config.serverIcon})
                .setDescription(`
                ${user} has Resigned from the staff team!
                `)
                .setThumbnail(config.serverIcon)]})
            }
        })
